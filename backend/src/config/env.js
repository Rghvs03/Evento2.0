...
export const env = {
  ...
  emailFrom: process.env.EMAIL_FROM || 'no-reply@evento.local',
  appName: process.env.APP_NAME || 'evento',
  cloudinaryCloudName: process.env.CLOUDINARY_CLOUD_NAME || '',
  cloudinaryApiKey: process.env.CLOUDINARY_API_KEY || '',
  cloudinaryApiSecret: process.env.CLOUDINARY_API_SECRET || '',
  // EMAIL_FROM can be overridden in .env; default includes the app name for clarity
  emailFrom: (process.env.EMAIL_FROM || `${process.env.APP_NAME || 'evento'} <no-reply@evento.local>`).toString(),
  // Sanitize APP_NAME; fallback to 'evento' if empty or missing
  appName: process.env.APP_NAME?.toString().trim() || 'evento',
};
